...
// WriteTo marshals Event to given Writer
func (ev *Event) WriteTo(w io.Writer) (bytesToWrite int64, err error) {
	...
	writeSize := 0
	if len(ev.Data) > 0 {
		if writeSize, err = fmt.Fprintf(w, "id: %s\n", ev.ID); err != nil {
			return
		}
		bytesToWrite += int64(writeSize)

		sd := bytes.Split(ev.Data, []byte("\n"))
		for i := range sd {
			if writeSize, err = fmt.Fprintf(w, "data: %s\n", sd[i]); err != nil {
				return
			}
			bytesToWrite += int64(writeSize)
		}
		...
		if len(ev.Event) > 0 {
			if writeSize, err = fmt.Fprintf(w, "event: %s\n", ev.Event); err != nil {
				return
			}
			bytesToWrite += int64(writeSize)
		}
		...
		if len(ev.Retry) > 0 {
			if writeSize, err = fmt.Fprintf(w, "retry: %s\n", ev.Retry); err != nil {
				return
			}
			bytesToWrite += int64(writeSize)
		}
	...
	if len(ev.Comment) > 0 {
		if writeSize, err = fmt.Fprintf(w, ": %s\n", ev.Comment); err != nil {
			return
		}
		bytesToWrite += int64(writeSize)
	}

	if writeSize, err = fmt.Fprint(w, "\n"); err != nil {
		return
	}
	bytesToWrite += int64(writeSize)
